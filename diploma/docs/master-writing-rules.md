...
Промежуточные разделы с заголовками вида `Выводы по главе`, `Выводы по разделу`, `Выводы по экспериментам` и аналогичными в основной части не используются. Локальные смысловые итоги встраиваются в основной текст соответствующей главы, а общий итог всей работы формулируется только в разделе `Заключение`, если пользователь явно не согласовал другую схему. (источник: пользовательские требования из переписки; diploma/text/08-conclusion.md)
...
Для таблиц действует дополнительное правило нормоконтроля: если в итоговом Word-документе таблица переносится на следующую страницу, над продолжением обязательно должна стоять строка `Продолжение таблицы N`, а при разбиении на последнюю часть при необходимости используется строка `Окончание таблицы N`. При этом шапка таблицы должна повторяться на каждой новой странице, чтобы продолжение можно было читать без возврата к предыдущему листу. (источник: diploma/docs/style-guide.md; diploma/source-materials/formatting-rules/форматирование.pdf)
...
Если таблица в `.docx` перешла на следующую страницу, в рамках нормоконтроля необходимо дополнительно проверить наличие надписи `Продолжение таблицы N`, повтор заголовочной строки и отсутствие ситуации, когда новая страница начинается непосредственно со строки данных без поясняющей подписи. Таблицы, которые не переносятся, не требуют такой служебной строки. (источник: diploma/docs/style-guide.md; diploma/source-materials/formatting-rules/форматирование.pdf)
