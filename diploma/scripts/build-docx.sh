...
DEFAULT_OUTPUT="ВКРБ_small_objects_auto_aug.docx"
...
#
# --syntax-highlighting=none: отключаем syntax highlighting в code blocks
# (исторический алиас --no-highlight депрекейтнут). По СТО СГАУ листинги в
# академической работе должны быть моноширинным чёрным текстом без цветовой
# подсветки. С флагом pandoc генерирует CodeBlock как обычный моноширинный
# текст под стилем Source Code из шаблона blank-template.docx.
pandoc \
    "${CHAPTERS[@]}" \
    -o "${OUTPUT}" \
...
# Pre-render TOC: гибрид Word-COM + inject-toc.py.
#  1) Копию docx открываем в Word headless через update-fields.ps1 — Word
#     добавляет _TocNNN-закладки в каждом заголовке и заполняет sdt-блок
#     TOC гиперссылками на эти закладки + номерами страниц.
#  2) inject-toc.py переносит из временной копии в основной pandoc-output
#     заполненный sdt-блок TOC, дублирует _Toc-закладки в заголовках
#     основного файла (точечные bookmarkStart/End с уникальными id) и
#     добавляет xmlns:w14, чтобы атрибуты sdt не дали "unbound prefix".
# Так в основном файле сохраняются styleId шаблона Маршуниной, а TOC
# становится статически заполнен — Word при открытии не показывает диалог
# "Обновить поля?" и сразу видит реальные заголовки и номера страниц.
PYBIN=""
...
UPDATE_FIELDS_REL="scripts/update-fields.ps1"
INJECT_TOC_REL="scripts/inject-toc.py"
if [[ -f "${UPDATE_FIELDS_REL}" && -f "${INJECT_TOC_REL}" && -n "${PYBIN}" ]] \
        && command -v powershell.exe >/dev/null 2>&1; then
    TOC_TMP="${OUTPUT%.docx}.toc-tmp.docx"
    cp "${OUTPUT}" "${TOC_TMP}"
    if powershell.exe -NoProfile -ExecutionPolicy Bypass \
            -File "${UPDATE_FIELDS_REL}" "${TOC_TMP}"; then
        "${PYBIN}" "${INJECT_TOC_REL}" "${TOC_TMP}" "${OUTPUT}"
    else
        echo "[build-docx] update-fields.ps1 упал; TOC остаётся пустым" >&2
    fi
    rm -f "${TOC_TMP}"
fi
