...
-- В markdown заголовки хранятся в Title Case ("Введение", не "ВВЕДЕНИЕ"),
-- чтобы TOC извлекал их в нормальном регистре. ПРОПИСНЫМИ они становятся
-- в финальном docx через character-level w:caps в Word-стиле "+ЗАГОЛОВОК по центру".
...
-- Нумерованные перечни в основном тексте: разворачиваем OrderedList в плоскую
-- последовательность Div'ов со стилем основного абзаца, префиксуя "N) " (N — порядковый
-- номер пункта в списке). Аналог bulletlist_to_blocks, но с цифровым маркером
-- вместо en-dash. Вызывается только вне раздела "Список использованных источников"
-- (там OrderedList разворачивается через refs_blocks_from).
local function orderedlist_to_blocks(ol)
    local out = {}
    local idx = 0
    for _, item in ipairs(ol.content) do
        ...
        for _, sub in ipairs(item) do
            if sub.t == "Para" or sub.t == "Plain" then
                local inlines = sub.content
                table.insert(inlines, 1, pandoc.Space())
                table.insert(inlines, 1, pandoc.Str(tostring(idx) .. ")"))
                table.insert(out, pandoc.Div(
                    { pandoc.Para(inlines) },
                    pandoc.Attr("", {}, { ["custom-style"] = BODY_PARAGRAPH_STYLE })
                ))
            ...
            elseif sub.t == "OrderedList" then
                for _, b in ipairs(orderedlist_to_blocks(sub)) do
                    table.insert(out, b)
                end
            else
                table.insert(out, sub)
...
function Pandoc(doc)
    ...
    for _, block in ipairs(doc.blocks) do
        ...
        elseif block.t == "OrderedList" then
            for _, b in ipairs(orderedlist_to_blocks(block)) do
                table.insert(new_blocks, b)
            end
        ...
        elseif block.t == "Figure" then
            -- Pandoc native Figure (от implicit_figures или явного figure-синтаксиса).
            -- Сборка идёт с --from=markdown-implicit_figures, так что эта ветка
            -- не должна срабатывать на штатных данных. Оставлена как защита: если
            -- флаг когда-нибудь отключат или появится явный figure-блок, captionу
            -- pandoc навешивает styleId "ImageCaption", который дублирует нашу
            -- ручную подпись "Рисунок N – ...". Поэтому распаковываем содержимое
            -- Figure (обычно Plain с одним Image) в обычный Para под стилем
            -- основного абзаца, а Caption отбрасываем.
            for _, inner in ipairs(block.content) do
                if inner.t == "Plain" then
                    table.insert(new_blocks, wrap_paragraph(pandoc.Para(inner.content)))
                elseif inner.t == "Para" then
                    table.insert(new_blocks, wrap_paragraph(inner))
                else
