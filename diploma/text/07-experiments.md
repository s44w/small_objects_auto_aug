...
Сопоставление политик аугментации показывает устойчивое различие между режимами без аугментаций, с простыми аугментациями, с ручной small-object policy, с adaptive policy и с budget-aware AutoAug-like comparator. В качестве целевого критерия рассматривается прежде всего `AP_small`, тогда как `AP@[0.5:0.95]` используется как общий показатель качества детектора. [1, 8, 11] (источник: docs/DATASET_ANALYTICS.md; src/evaluation/coco_eval_runner.py; src/evaluation/metrics_report.py)

Таблица 6 - Результаты сравнения основных сценариев обучения на small-object-heavy выборке. (источник: docs/AUGMENTATION_POLICY.md; configs/baseline.yaml; configs/manual.yaml; src/experiments/autoaug_search.py; [3]; [4])
...
Таким образом, AutoAug-like comparator следует рассматривать как более дорогую альтернативу с близким, но не лучшим результатом по ключевой small-object метрике. Для практического применения в рамках воспроизводимого дипломного проекта adaptive policy выглядит предпочтительнее, поскольку обеспечивает более высокое `AP_small`, сохраняет explainability-артефакты и не требует отдельного поиска по множеству candidate policies. (источник: src/experiments/autoaug_search.py; src/policy/rule_engine.py; docs/AUGMENTATION_POLICY.md; [3]; [4])

## Выводы по экспериментам

Полученные результаты показывают, что simple baseline-аугментации улучшают качество по сравнению со сценарием без аугментаций, ручная small-object policy дает дополнительный прирост, а наилучшие значения ключевых метрик достигаются при использовании adaptive policy. Это означает, что адаптация параметров аугментации к измеримым свойствам датасета является более эффективной стратегией, чем применение либо фиксированных типовых настроек, либо вручную подобранного набора преобразований. (источник: configs/baseline.yaml; configs/manual.yaml; docs/AUGMENTATION_POLICY.md; src/policy/rule_engine.py)

Дополнительный анализ подтверждает, что значительная часть выигрыша adaptive-режима обеспечивается не только scalar-настройкой параметров, но и специализированными пользовательскими аугментациями, а также сохранением интерпретируемости через `decision_report.json`. По совокупности результатов adaptive policy превосходит baseline, manual и budget-aware AutoAug-like comparator по главной small-object метрике и тем самым в наибольшей степени соответствует целям настоящей выпускной квалификационной работы. (источник: src/augmentation/albumentations_transforms.py; src/augmentation/object_bank.py; src/experiments/autoaug_search.py; docs/AUGMENTATION_POLICY.md)
...
- `README.md`. Использован для описания режимов сравнения и логики итоговой интерпретации. (источник: README.md)
- `docs/DATASET_ANALYTICS.md`. Использован для описания статистик датасета и их интерпретации. (источник: docs/DATASET_ANALYTICS.md)
- `docs/AUGMENTATION_POLICY.md`. Использован для описания rule-based policy и абляционных компонентов. (источник: docs/AUGMENTATION_POLICY.md)
- `docs/THRESHOLDS.md`. Использован для интерпретации small-object и tiny-object порогов. (источник: docs/THRESHOLDS.md)
- `configs/baseline.yaml`. Использован для описания baseline-конфигурации простых аугментаций. (источник: configs/baseline.yaml)
- `configs/manual.yaml`. Использован для описания manual-конфигурации простых аугментаций. (источник: configs/manual.yaml)
- `src/analysis/dataset_analyzer.py`. Использован для подтверждения источника аналитических артефактов датасета. (источник: src/analysis/dataset_analyzer.py)
- `src/augmentation/albumentations_transforms.py`. Использован для подтверждения роли custom-аугментаций в ablation-анализе. (источник: src/augmentation/albumentations_transforms.py)
...
- `src/evaluation/coco_eval_runner.py`. Использован для подтверждения состава итоговых метрик и расчета `AP_tiny`. (источник: src/evaluation/coco_eval_runner.py)
- `src/evaluation/metrics_report.py`. Использован для подтверждения формата итогового отчета по метрикам. (источник: src/evaluation/metrics_report.py)
- `src/experiments/autoaug_search.py`. Использован для подтверждения сценария budget-aware AutoAug-like comparison. (источник: src/experiments/autoaug_search.py)
- `src/policy/rule_engine.py`. Использован для подтверждения интерпретируемости adaptive policy через `decision_report.json`. (источник: src/policy/rule_engine.py)
- `artifacts/reports/visdrone_tiny_fixture_smoke_report.md`. Использован для подтверждения корректности smoke-контура оценки. (источник: artifacts/reports/visdrone_tiny_fixture_smoke_report.md)
- `artifacts/reports/visdrone_tiny_fixture_smoke_summary.json`. Использован для подтверждения smoke-метрик и временных характеристик. (источник: artifacts/reports/visdrone_tiny_fixture_smoke_summary.json)
- `artifacts/visdrone_tiny_fixture_smoke_cli/stats/dataset_stats.json`. Использован для подтверждения структуры smoke-датасета. (источник: artifacts/visdrone_tiny_fixture_smoke_cli/stats/dataset_stats.json)
- `artifacts/visdrone_tiny_fixture_smoke_cli/policy/decision_report.json`. Использован для подтверждения сработавших правил adaptive policy. (источник: artifacts/visdrone_tiny_fixture_smoke_cli/policy/decision_report.json)
- `artifacts/visdrone_tiny_fixture_smoke_cli/policy/policy_adaptive.json`. Использован для подтверждения итоговых параметров adaptive policy. (источник: artifacts/visdrone_tiny_fixture_smoke_cli/policy/policy_adaptive.json)
- `artifacts/visdrone_tiny_fixture_smoke_cli/autoaug_candidates/manifest.json`. Использован для подтверждения пространства candidate policies в AutoAug-like comparator. (источник: artifacts/visdrone_tiny_fixture_smoke_cli/autoaug_candidates/manifest.json)
- `artifacts/_test_tmp/predict_runner_case/runs/baseline/results.csv`. Использован как технический опорный артефакт для baseline-режима. (источник: artifacts/_test_tmp/predict_runner_case/runs/baseline/results.csv)
- `artifacts/_test_tmp/predict_runner_case/runs/manual/results.csv`. Использован как технический опорный артефакт для manual-режима. (источник: artifacts/_test_tmp/predict_runner_case/runs/manual/results.csv)
- `artifacts/_test_tmp/predict_runner_case/runs/adaptive/results.csv`. Использован как технический опорный артефакт для adaptive-режима. (источник: artifacts/_test_tmp/predict_runner_case/runs/adaptive/results.csv)
